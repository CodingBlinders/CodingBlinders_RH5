...
const ChatBot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (userInput.trim() !== '') {
      ...
      try {
        ...
        // http://172.208.88.139:8081/chat
        // const response = await fetch("http://20.235.249.223:8000/free", requestOptions);
        const response = await fetch("http://172.208.88.139:8081/chat", requestOptions);
        if (!response.ok) {
  ...
  return (
    <div className="chat-container w-1/2 bg-blue-300 mx-auto">
